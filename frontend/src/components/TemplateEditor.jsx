...
export default function TemplateEditor({ onClose, onCreated }) {
  ...
  function validate() {
    ...
    if (headerType !== 'NONE' && headerType !== 'TEXT' && !headerMediaUrl) return 'Header media is required.';
    ...
    try {
      ...
      if (andSend) {
        try {
          ...
          setErrorMsg('Saved as DRAFT but Meta submission failed: ' + (e.response?.data?.error || e.message));
          onCreated?.();
          return;
        }
      }
  ...
  return (
    <div className="absolute inset-0 bg-white flex flex-col z-20">
      <div className="px-4 py-3 bg-wati-primaryDark text-white flex items-center justify-between">
      ...
      <div className="flex-1 overflow-y-auto thin-scroll p-4 space-y-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select value={language} onChange={e => setLanguage(e.target.value)} className="w-full mt-1 px-3 py-2 bg-gray-100 rounded outline-none text-sm">
        ...
        <div>
          ...
          <div className="flex gap-2 mt-1">
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setCategory(c)} className={`px-3 py-1.5 rounded text-xs border ${category === c ? 'bg-wati-primary text-white border-wati-primary' : 'bg-gray-100 border-gray-200'}`}>{c}</button>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2 mt-1">
            {['NONE', 'TEXT', 'IMAGE', 'VIDEO', 'DOCUMENT'].map(t => (
              <button key={t} onClick={() => setHeaderType(t)} className={`px-3 py-1.5 rounded text-xs border ${headerType === t ? 'bg-wati-primary text-white border-wati-primary' : 'bg-gray-100 border-gray-200'}`}>{t}</button>
            ))}
          </div>
          {headerType === 'TEXT' && (
            <input value={headerText} onChange={e => setHeaderText(e.target.value)} placeholder="Header text (up to 60 chars)" maxLength={60} className="w-full mt-2 px-3 py-2 bg-gray-100 rounded outline-none text-sm" />
          )}
          {['IMAGE', 'VIDEO', 'DOCUMENT'].includes(headerType) && (
            <div className="mt-2">
              <label className="inline-flex items-center gap-2 px-3 py-2 bg-gray-100 rounded cursor-pointer text-sm hover:bg-gray-200">
                ...
                <input type="file" hidden accept={headerType === 'IMAGE' ? 'image/*' : headerType === 'VIDEO' ? 'video/*' : '.pdf,.doc,.docx'} onChange={onHeaderFile} />
              </label>
              {headerMediaUrl && (
                <div className="mt-2">
        ...
        <div>
          ...
          <textarea value={body} onChange={e => setBody(e.target.value)} rows={5} placeholder="Hello {{1}}, your order {{2}} is confirmed."
        ...
        <div>
          ...
          <input value={footer} onChange={e => setFooter(e.target.value)} maxLength={60} className="w-full mt-1 px-3 py-2 bg-gray-100 rounded outline-none text-sm" />
        </div>

        <div>
          <div className="flex items-center justify-between">
            <label className="text-xs text-wati-muted">Buttons (up to 3)</label>
            {buttons.length < 3 && <button onClick={addButton} className="text-xs text-wati-primary flex items-center gap-1"><Plus size={14} /> Add button</button>}
          </div>
          <div className="space-y-2 mt-2">
            {buttons.map((b, i) => (
              <div key={i} className="border rounded p-2 space-y-2">
                <div className="flex items-center gap-2">
                  <select value={b.type} onChange={e => updateButton(i, { type: e.target.value })} className="px-2 py-1.5 bg-gray-100 rounded text-xs">
                    <option value="QUICK_REPLY">Quick reply</option>
                    <option value="URL">URL</option>
                    <option value="PHONE_NUMBER">Call</option>
                  </select>
                  <input value={b.text || ''} onChange={e => updateButton(i, { text: e.target.value })} placeholder="Button text" maxLength={25} className="flex-1 px-2 py-1.5 bg-gray-100 rounded text-sm" />
                  {b.type === 'URL' && <input value={b.url || ''} onChange={e => updateButton(i, { url: e.target.value })} placeholder="https://…" className="flex-1 px-2 py-1.5 bg-gray-100 rounded text-sm" />}
                  {b.type === 'PHONE_NUMBER' && <input value={b.phone_number || ''} onChange={e => updateButton(i, { phone_number: e.target.value })} placeholder="+919999999999" className="flex-1 px-2 py-1.5 bg-gray-100 rounded text-sm" />}
                  <button onClick={() => removeButton(i)} className="p-1 text-red-500 hover:bg-red-50 rounded"><Trash2 size={14} /></button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="px-4 py-3 border-t flex items-center justify-end gap-2 bg-gray-50">
        ...
        <button onClick={() => submit(false)} disabled={submitting} className="px-4 py-2 text-sm bg-gray-700 text-white rounded disabled:opacity-50">Save as Draft</button>
        <button onClick={() => submit(true)} disabled={submitting} className="px-4 py-2 text-sm bg-wati-primary text-white rounded disabled:opacity-50">{submitting ? 'Submitting…' : 'Create & Verify'}</button>
      </div>
    </div>
  );
}
